...
**CRM-сервис с контактами, использующий схема-ориентированную мультиарендность 
для изоляции данных организаций.**

## Затраченное время

- Общее время выполнения: 14.1 ч. (846 мин.) + 60 мин. рефакторинг `README.md` после созвона

## 🚧 Текущие недоработки

В текущей версии проекта есть следующие недоработки:

1. **Админка не полностью настроена** - работа с моделью `contacts_contact` 
отваливается в админке из-за особенностей мультитенантной архитектуры

2. **Нет деплоя в Docker Hub** - настроен только локальный `Docker`, 
без деплоя в `Docker Hub` и без настройки `GitHub Actions` с удаленным хостом

3. **Нет Celery задач** - не реализованы `Celery` задачи из бонусных заданий, 
хотя `Docker Compose` для `Celery` уже создан и настроен
...
## Подробная документация по тестированию мультитенантной архитектуры: [Документация по тестированию](src/tests/README.md)
