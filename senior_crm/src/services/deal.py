...
from src.models import Deal, User, DealStatus, DealStage, UserRole
from src.services.organization import OrganizationService


# Stage order for validation
STAGE_ORDER = [DealStage.QUALIFICATION, DealStage.PROPOSAL, DealStage.NEGOTIATION, DealStage.CLOSED]
...
class DealService:
    ...
    async def update_deal(
        ...
        **kwargs,
        ...
        if status is not None:
            await self._validate_status_change(deal, status, kwargs.get("amount"))

        # Validate stage change
        if stage is not None:
            self._validate_stage_change(deal, stage, member)
    ...
    async def _validate_status_change(
        ...
        # Rule: Cannot close as won if amount <= 0
    ...
    def _validate_stage_change(
        ...
        # Rule: No rollback for members (only admin/owner can rollback)
        if new_idx < old_idx:
